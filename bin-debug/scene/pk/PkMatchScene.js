...
var PkMatchScene = (function (_super) {
    ...
    PkMatchScene.prototype.init = function () {
        ...
        icon.mask = circle;
        group.addChild(icon);
        var circleBg = Util.createBitmapByName('pk_circle_bg_png');
        circleBg.width = 326;
        circleBg.height = 326;
        ...
        //  egret.Tween.get( circleBg, { loop:true} ) .to( {	rotation :360},3000);
        var matching = new egret.TextField();
        matching.text = "匹配中....";
        matching.width = 200;
        matching.size = 36;
        ...
        //     }
        // }, this);
        var userName = new LineInfo("用户名:" + user.nickName);
        userName.y = y;
        group.addChild(userName);
        y += 100;
        var userTeam = new LineInfo("来自:" + user.teamName);
        userTeam.y = y;
        group.addChild(userTeam);
        y += 100;
        var userLevel = new LineInfo("\u7B49\u7EA7:" + user.lvName + user.lvShow);
        userLevel.y = y;
        group.addChild(userLevel);
        var buttonY = this.stage.stageHeight - 200;
        var cancelButton = new XButton("取消匹配");
 ...
 * 匹配成功页面
 */
var PkMatchSuccessScene = (function (_super) {
    __extends(PkMatchSuccessScene, _super);
    function PkMatchSuccessScene() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    PkMatchSuccessScene.prototype.init = function () {
        this.width = 750;
        ...
        var rightUser = new PkUser(userinfo, 'right');
        rightUser.y = 100;
        ...
        vs.size = 40;
        this.addChild(vs);
        var matchText = new egret.TextField();
        matchText.width = 750;
        ...
        var alert = new LineInfo("5s后开始游戏");
        alert.y = 700;
        this.addChild(alert);
    };
    return PkMatchSuccessScene;
}(Scene));
__reflect(PkMatchSuccessScene.prototype, "PkMatchSuccessScene");
